...
void setup() {
  ...
  if (config.flags.bluetoothEnable) {
    spiController.init();
  }
...
void loop() {
  ...
  if (config.flags.lowLatencyMode) {
    if (toggle == 0) {
      plunger.plungerRead();
  ...
  if (config.flags.updateUSB || buttons.numberButtonsPressed > 0) {
    buttons.readInputs();
    Gamepad1.write();
    config.flags.updateUSB = false;
    config.flags.buttonPressed = false;
    // unsigned long t2 = micros();
